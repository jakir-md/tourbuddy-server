...
export const sslPaymentInit = async (payload: IsslCommerz) => {
  try {
    const data = {
      store_id: EnvVars.SSL_STORE_ID as string,
      store_passwd: EnvVars.SSL_STORE_PASS as string,
      total_amount: payload.amount as number,
      currency: "BDT",
      tran_id: payload.transactionId as string,
